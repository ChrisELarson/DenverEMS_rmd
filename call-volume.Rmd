...
month_year_plot <- month_year %>% 
  ...
  xlab("Month") +
  ylab("Total Calls")
div(ggplotly(month_year_plot))  #the div() wrapper from shiny keeps plot centered.  fig.align off with plotly
```
...
Expanding the pool of available part time employees increases the chances of availabilty to fill open shifts, particularly during the busiest times of the year.  There is less associated overhead as employees are only paid for hours worked with minimal benefit considerations.  To maintain a large intermittent employee population there would potentially need to be a relaxing of current rules that dictate the amount of hours required to work in a specific time period. One of the possible drawbacks of large numbers of part time employees as a solution to increased call volumes include the chance that there aren't enough employees available or willing to cover the necessary shifts to meet demand. 
