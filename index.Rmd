...
```{r hranalysis}
# create data set for overall mean calls per hour, Hour needs change to numeric for ggplot
num_hr <- call_data %>% 
  filter(Year != 2019) %>% 
  group_by(Hour) %>% 
  summarise(calls = ceiling(mean(calls)))
num_hr$Hour <- as.numeric(num_hr$Hour)

# Mean calls per hour of specific day vs. overall mean calls per hour
mean_day <- call_data %>%  
  filter(Year != 2019, wday == 'Fri') %>% 
  group_by(Hour) %>% 
  summarise(calls = ceiling(mean(calls))) %>%
  ggplot(aes(x = Hour, y = calls)) +
  geom_col(fill = 'lightblue', color = 'black') +
